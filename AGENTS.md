...
- The CI step (`internal/pipeline/steps/ci.go`) babysits an open PR until it is merged, closed, the run is cancelled, or `ci_timeout` elapses. It auto-fixes failing checks and rebases on merge conflicts via `autoFixCI`. A PR that never reports a single CI check within `noChecksCompletionWindow` (default 3m) completes the step instead of monitoring until merge - there is no CI to watch; the guard is `checksSeen`, so checks that were observed and later disappear keep the merge-or-close monitor (`TestCIStep_CompletesWhenNoChecksRegisterWithinWindow`, `TestCIStep_KeepsMonitoringWhenObservedChecksDisappear`).
- `ci_timeout` is an **idle timeout, not an absolute deadline**: it re-arms (`timeoutAnchor = now()`) every time the upstream default-branch tip advances, so an actively-rebased green PR keeps its monitor no matter how long it stays open. `started` stays fixed for poll-interval pacing, while the no-checks registration window is anchored independently by `emptyChecksSince`; only `timeoutAnchor` moves when the base advances. Re-arm only ever extends the deadline, so a transient base-tip resolution failure is fail-safe. `baseBranchTip` is injectable for tests.
