...
When the CI step is still monitoring an open PR and checks are green, or no checks are reported after the 60-second registration grace period, `axi run` exits successfully with `outcome: checks-passed` instead of waiting for a human merge.
