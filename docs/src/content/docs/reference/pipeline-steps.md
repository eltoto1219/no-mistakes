...
- Continues monitoring an open PR that has reported CI checks until it is merged, closed, declined, or the configured `ci_timeout` idle window elapses, even after CI checks are currently healthy
- Completes the step after the three-minute registration window if the PR never reports a CI check, including when `ci_timeout: "unlimited"`
- Treats `ci_timeout` as an idle timeout: each upstream default-branch advance re-arms the timer, and `ci_timeout: "unlimited"` disables timeout-based termination
...
- Keeps monitoring when previously observed checks disappear, preserving the ready signal until the PR is merged or closed
